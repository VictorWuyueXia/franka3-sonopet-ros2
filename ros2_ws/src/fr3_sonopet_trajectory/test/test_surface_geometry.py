...
def test_fit_surface_frame_recovers_plane_and_faces_base_z():
    ...
    frame = fit_surface_frame(patch, center_xyz=np.array([0.0, 0.0, 0.2]))

    assert np.allclose(frame.normal, (0.0, 0.0, 1.0))
    assert np.allclose(frame.tangent, (1.0, 0.0, 0.0))
